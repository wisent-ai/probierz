 ...
 *
 * The other three groups this file used to hold are beside it now:
 * `jeden.functional.spec.ts` for what a command does, `jeden.discovery.spec.ts`
 * for what the app finds in a workspace, and `jeden.content.spec.ts` for what
 * each read-only view must contain.
 */
