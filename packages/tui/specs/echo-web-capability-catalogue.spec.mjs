...
const envFile = process.env.ECHO_GUI_TEST_ENV?.trim();
assert.ok(envFile, 'ECHO_GUI_TEST_ENV is required; see the echo-web-gui manifest prerequisites');
assert.ok(isAbsolute(envFile), 'ECHO_GUI_TEST_ENV must be an absolute path');
assert.ok(statSync(envFile).isFile(), 'ECHO_GUI_TEST_ENV must identify a readable file');
...
for (const name of ['NEXT_PUBLIC_SUPABASE_URL', 'NEXT_PUBLIC_SUPABASE_ANON_KEY', 'SUPABASE_SERVICE_ROLE_KEY']) {
  assert.ok(configured[name], `${name} is required in ECHO_GUI_TEST_ENV`);
}
...
assert.equal(result.status, 0, [result.stdout, result.stderr, logs.join('')].filter(Boolean).join('\n') || `Echo GUI test exited ${result.status}`);

const evidencePath = join(process.env.PROBIERZ_ARTIFACTS || 'test-results', 'echo-web-capability-catalogue.json');
await mkdir(join(evidencePath, '..'), { recursive: true });
await writeFile(evidencePath, `${JSON.stringify({
  command: 'npm run test:capability-gui',
  repository,
  assertions: ['real approved admin', 'protected GUI', '11 workspaces', 'documentation link', 'CLI commands'],
}, null, 2)}\n`, { mode: 0o600 });
