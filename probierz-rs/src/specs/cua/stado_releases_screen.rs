...
pub fn run(context: &specs::Context) -> Result<(), String> {
    ...
    let result = (|| {
        ...
        let quarantined = quarantined.ok_or_else(|| {
            ...
            format!("no rollout on this fleet holds a quarantined digest, so the screen's only write could not be reached; tree: {}", super::common::tail(&tree, 2500))
        })?;
            ...
            .contains("REQUIRED, RECORDED IN THE AUDIT TRAIL")
        {
            return Err(format!(
                ...
                super::common::tail(&dialog.tree, 2000)
            ));
        }
